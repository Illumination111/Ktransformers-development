...
- KTransformers source: `/mnt/data2/wbw/ktransformers/kt-kernel`;
...
| LLaMA-Factory source | Pass (development source) | `qwen3_vl` template, `qwen3_vl_moe` composite registration, ordinary text-side VLM LoRA, and KT Conv3D marker validation are present |
| KT Conv3D | Pass | Instance-scoped torch 2.9.1 fallback self-test passed with matching forward/backward results |
...
bash run_vlm_lora_smoke.sh --dry-run --lora-scope text
```

The shared local Conv3D and cross-repository contract suite passes. Detailed
scope experiments from the earlier branch are not part of the current PR.
...
`OPTIMIZER_OK`, and `PASS`, then save a text-side adapter.
