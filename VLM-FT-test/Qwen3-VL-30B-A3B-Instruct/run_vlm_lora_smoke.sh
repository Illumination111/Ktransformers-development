#!/usr/bin/env bash
# Staged Qwen3-VL-30B-A3B VLM LoRA smoke test. Preflight and dry-run do
...
export VLM_EXPECTED_KT_WRAPPERS=48
export VLM_KT_ARCH_COMPAT="${KT_SOURCE_DIR}/python/sft/arch.py"
...
if [[ "${LORA_SCOPE}" == "vision" && "${DRY_RUN}" -eq 0 ]]; then
    "${PYTHON}" -c 'from kt_kernel.sft.config import KTConfig; raise SystemExit(0 if "kt_freeze_experts" in KTConfig.__dataclass_fields__ else 1)' ||
        die "vision scope requires kt-kernel with kt_freeze_experts support"
fi
...
"${PYTHON}" "${SCRIPT_DIR}/render_train_config.py" \
    --template "${CONFIG_DIR}/train_vlm_lora_qwen3vl_30b.yaml.template" \
    --output "${TRAIN_CONFIG}" \
    --model-path "${MODEL_PATH}" \
    --dataset-dir "${DATASET_DIR}" \
    --dataset-name "${DATASET_NAME}" \
...
"${PYTHON}" "${TEST_ROOT}/resource_monitor.py" --output "${RESOURCE_SAMPLES}" --devices "${DEVICES}" --interval 1 &
RESOURCE_MONITOR_PID=$!
...
"${PYTHON}" "${TEST_ROOT}/summarize_resources.py" \
    --input "${RESOURCE_SAMPLES}" --output "${RUN_DIR}/resource_summary.json" --require-gpu \
...
"${PYTHON}" "${SCRIPT_DIR}/validate_adapter_output.py" \
    --output-dir "${RUN_DIR}/model_output" --lora-scope "${LORA_SCOPE}" \
