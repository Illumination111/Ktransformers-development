...
def torch_minor(version: str) -> tuple[int, int] | None:
...
def validate_model(model_path: Path) -> tuple[Any, dict[str, Any]]:
    ...
    for name in ("preprocessor_config.json", "video_preprocessor_config.json", "tokenizer.json"):
    ...
    if (moe.expert_num, moe.intermediate_size, moe.num_experts_per_tok) != (256, 1024, 8):
...
def validate_dataset(dataset_dir: Path, dataset_name: str) -> tuple[Path, list[dict[str, Any]], list[Path]]:
    ...
    if not isinstance(entry, dict):
        fail(f"dataset {dataset_name!r} is not registered in {dataset_dir / 'dataset_info.json'}")
    file_name = entry.get("file_name")
    if not file_name:
        fail(f"dataset {dataset_name!r} is hub-only; this smoke test requires a local file_name")
    ...
    if placeholder_count != len(image_paths):
        fail(f"image placeholder/reference mismatch: placeholders={placeholder_count}, images={len(image_paths)}")
...
def validate_processor(model_path: Path, image_path: Path, config_raw: dict[str, Any]) -> dict[str, Any]:
    from PIL import Image
    ...
    if batch["pixel_values"].ndim != 2 or batch["pixel_values"].shape[-1] != patch_width:
        fail(f"unexpected pixel_values shape: {tuple(batch['pixel_values'].shape)}; expected width {patch_width}")
    return {
...
def main() -> int:
    ...
    parser.add_argument("--allow-torch29-conv3d", action="store_true")
    ...
    version_pair = torch_minor(torch.__version__)
    conv3d_risk = version_pair == (2, 9)
    if args.require_cuda and not torch.cuda.is_available():
        fail("CUDA is not visible; an actual 122B smoke run requires eight visible GPUs")
    if conv3d_risk and not args.allow_torch29_conv3d:
        fail(
            f"torch {torch.__version__} is in the known Conv3D+AMP regression window; "
            "use torch <2.9 or >=2.10, or pass --allow-torch29-conv3d only for an explicit diagnostic run"
        )
    ...
    summary = {
        ...
        "torch29_conv3d_risk_accepted": conv3d_risk and args.allow_torch29_conv3d,
        **processor_summary,
    }
