"""Qwen3.5-122B-A10B text-only BF16 benchmark entrypoint (KTransformers/DeepSpeed)."""
...
EXPECTED_ARCHITECTURE = "Qwen3_5MoeForCausalLM"
EXPECTED_LOGICAL_PARAMETERS = 122_111_526_912
EXPECTED_ROUTED_EXPERT_PARAMETERS = 115_964_116_992
EXPECTED_KT_WRAPPERS = 48
EXPECTED_TORCH_OWNED_LAYERS = 6
EXPECTED_TORCH_OWNED_EXPERT_PARAMETERS = EXPECTED_ROUTED_EXPERT_PARAMETERS // 8
...
def _validate_full_finetuning_contract(model) -> dict[str, int]:  # type: ignore[no-untyped-def]
    ...
    if len(wrappers) != EXPECTED_KT_WRAPPERS:
        raise RuntimeError(
            "Unexpected KTransformers MoE wrapper coverage: "
    ...
    if placeholder_numel != EXPECTED_ROUTED_EXPERT_PARAMETERS:
        raise RuntimeError(
            "Unexpected routed-expert placeholder coverage: "
            f"got={placeholder_numel}, "
            f"expected={EXPECTED_ROUTED_EXPERT_PARAMETERS}"
        )
...
def _validate_pytorch_torch_full_finetuning_contract(
    ...
    if getattr(model, "_kt_train_mode", None) != "full" or not getattr(model, "_kt_full_weight_grad", False):
        raise RuntimeError(
    ...
    registered_trainable = sum(
        parameter.numel() for _, parameter in model.named_parameters() if parameter.requires_grad
    )
...
def _training_backend() -> str:
    ...
    if backend not in SUPPORTED_BACKENDS:
        raise RuntimeError(
            "Qwen3.5-122B-A10B entrypoint requires backend "
...
def _configure_rank_threads() -> None:
    ...
    if backend in {"deepspeed", "pytorch_cuda", "pytorch_torch"}:
        ...
        rank = int(os.environ.get("RANK", os.environ.get("LOCAL_RANK", "0")))
        print(
            f"[qwen35_122b_bf16_threads] rank={rank} role={backend} "
    ...
    print(
        f"[qwen35_122b_bf16_threads] rank={rank} "
...
def _install_model_contract() -> None:
    """Validate text-only architecture and full-FT coverage before optimizer setup."""
    import llamafactory.model as model_api
    from llamafactory.model import loader

    original_load_model = loader.load_model

    def load_qwen_text_model(*args, **kwargs):  # type: ignore[no-untyped-def]
        ...
        if int(os.environ.get("LOCAL_RANK", "0")) == 0:
            print(
                "[qwen35_122b_model_contract] contract=OK "
                f"backend={backend} "
    ...
    loader.load_model = load_qwen_text_model
    model_api.load_model = load_qwen_text_model
...
def _install_timing() -> None:
    ...
    print(
        f"[qwen35_122b_bf16_timing] rank={rank}/{world_size} "
...
def _install_cuda_memory_workaround() -> None:
    """Release FSDP2 preparation cache before the first training step."""
...
def _disable_benchmark_saves() -> None:
    ...
    def skip_save_model(self, *args, **kwargs):  # type: ignore[no-untyped-def]
        if self.is_world_process_zero():
            print("[qwen35_122b_bf16] final model save skipped", flush=True)

    def skip_save_state(self, *args, **kwargs):  # type: ignore[no-untyped-def]
        if self.is_world_process_zero():
            print("[qwen35_122b_bf16] trainer state save skipped", flush=True)
