#!/usr/bin/env bash
# Qwen3.5-122B-A10B PyTorch TORCH full-FT using the accepted sap4 stack.
...
MAIN_PORT="${FFT_TORCH_MAIN_PORT:-29841}"
...
check_files_and_environment() {
    ...
    for required in \
        ...
        "${SCRIPT_DIR}/qwen35_text_only.py" \
...
run_one_sequence() {
    ...
    local case_unit="fft-qwen35-122b-pytorch-torch-seq${seq}-$$"
...
log "Qwen3.5-122B-A10B PyTorch TORCH full-FT; xmy venv is read-only"
