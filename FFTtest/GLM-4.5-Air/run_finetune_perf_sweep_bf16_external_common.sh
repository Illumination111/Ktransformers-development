#!/usr/bin/env bash
# Shared GLM-4.5-Air native-BF16 full-finetuning sweep for DeepSpeed/MegaTrain.
...
MEGATRAIN_CONFIG_BASE="${CONFIGS_DIR}/megatrain_glm45_air_bf16.yaml"
MEGATRAIN_ENTRYPOINT="${SCRIPT_DIR}/megatrain_glm45_air_train.py"
...
FFT_RUNTIME_ROOT="${FFT_RUNTIME_ROOT:-/mnt/data2/wbw/fft_runtime/glm45_air/${BACKEND}}"
...
Options:
  ...
  --devices LIST          Physical GPU ids; profile uses the first N ids
...
if (( SEQUENCE_LENGTHS_OVERRIDE_SET == 0 )); then
    case "${PROFILE}" in
...
profile_parameters() {
    ...
    case "${profile_name}" in
        server) NUM_GPUS=8; GLOBAL_BATCH_SIZE=8 ;;
        consumer) NUM_GPUS=2; GLOBAL_BATCH_SIZE=2 ;;
...
check_numa_capacity() {
    command -v numactl >/dev/null || die "numactl is required for consumer"
    local node total_kib
    local -a nodes
    IFS=',' read -r -a nodes <<< "${CONSUMER_NUMA_NODES// /}"
    (( ${#nodes[@]} == 2 )) || die "consumer requires exactly two NUMA nodes"
...
obj = {
    ...
    "result_scope": (
        "end-to-end GLM-4.5-Air full-finetune throughput via MegaTrain; "
        "backend CUDA synchronization retained"
        if backend == "megatrain"
        else "end-to-end GLM-4.5-Air full-finetune throughput via DeepSpeed"
    ),
}
...
log "GLM-4.5-Air full-finetuning sweep: backend=${BACKEND}, profile=${PROFILE}, precision=BF16"
