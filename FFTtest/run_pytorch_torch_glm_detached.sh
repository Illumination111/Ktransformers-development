#!/usr/bin/env bash
# Detach GLM-4.5-Air PyTorch TORCH full-FT sweep. Survives terminal hangup.
# Short sequences run first so a slow/failing 1024 cannot block 32–512.
# Does not touch /mnt/data2/xmy/venv.
...
SEQ_LENGTHS="${FFT_TORCH_SEQ_LENGTHS:-32,64,128,256,512,1024}"
...
MASTER_DIR="${LOG_BASE}/${MASTER_STAMP}_PYTORCH_TORCH_GLM_SWEEP"
...
run_chain() {
    echo "[$(date '+%F %T')] start GLM-4.5-Air seq=${SEQ_LENGTHS} steps=${STEPS} warmup=${WARMUP_STEPS}"
