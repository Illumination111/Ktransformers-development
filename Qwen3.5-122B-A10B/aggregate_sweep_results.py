...
def write_summary(root: Path, rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Qwen3.5-122B-A10B Text-Only BF16 Full-Finetuning Sweep",
        "",
        ...
        "- CPU memory shown below is the dedicated cgroup v2 `memory.current` peak.",
        "- Process-tree RSS remains in `monitor.csv`, `memory_summary.json`, and `sweep_results.csv` for diagnostics only.",
        ...
        "",
        "| GPUs | Seq | Status | Stable steps | Step sec | TPS | Forward | Backward | Optimizer | CPU cgroup peak GB | GPU peak GiB |",
        "|---:|---:|:---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            "| {num_gpus} | {sequence_length} | {status} | {stable_steps} | "
            "{mean_step_sec} | {stable_tps} | {forward_sec} | "
            "{backward_sec} | {optimizer_sec} | "
            "{cgroup_memory_peak_gb} | {max_gpu_task_peak_gib} |".format(
