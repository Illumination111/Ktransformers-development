#!/usr/bin/env bash
# Qwen3.5-122B-A10B text-only native-BF16 full-finetuning sequence sweep.
# Scope is intentionally fixed to the KTransformers server profile.
...
The multimodal source checkpoint is loaded strictly as the text-only
Qwen3_5MoeForCausalLM. Vision, processor, and MTP weights are excluded.
Every sequence length runs in an independent process. Timing records only
coarse forward, backward, optimizer and total host-wall phases without forced
CUDA synchronization. CPU/GPU sampling runs outside the measured phase path.
EOF
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --seq-lengths)
            ...
            ;;
        --seq-length)
            need_value "$1" "$#"; SEQUENCE_LENGTH="$2"
...
[[ "${SEQUENCE_LENGTHS_OVERRIDE_SET}" -eq 1 ]] || MAX_SEQUENCE_LENGTH=4096
...
check_files_and_environment() {
    ...
    for required in \
        ...
        "${MONITOR_SCRIPT}" "${MEMORY_ANALYZER}" "${STEP_PHASE_TIMER}" \
    ...
    env MPLCONFIGDIR=/tmp/fft_qwen35_122b_matplotlib \
...
obj = {
    ...
    "result_scope": (
        "end-to-end Qwen3.5-122B-A10B text-only full-finetune throughput"
    ),
}
out.write_text(
...
if [[ "${SKIP_DATASET_CHECK}" -eq 0 ]]; then
    log "Validating Qwen3.5-122B-A10B text-only BF16 checkpoint and dataset lengths"
    "${PYTHON}" "${VALIDATOR}" \
...
log "Qwen3.5-122B-A10B text-only full-finetuning sweep: backend=ktransformers, profile=server, precision=BF16"
