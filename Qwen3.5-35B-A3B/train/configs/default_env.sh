# Shared defaults for Qwen3.5-35B-A3B LoRA SFT (KT) training.
...
MODEL_CASE_DIR="${MODEL_CASE_DIR:-${MLS_ROOT}/Qwen3.5-35B-A3B}"
...
MODEL_PATH="${MODEL_PATH:-/mnt/data3/models/Qwen3.5-35B-A3B}"

# Raw HF downloads (shared with the 397B case under MLStest/dataset)
DATASET_ROOT="${DATASET_ROOT:-${MLS_ROOT}/dataset}"
# Prepared openai jsonl + dataset_info.json (per-case). Existing jsonl may be
# symlinked from Qwen3.5-397B-A17B/train/data to avoid duplicating large files.
...
ADAPTER_ROOT="${ADAPTER_ROOT:-${MLS_ROOT}/lora-adapter/Qwen3.5-35B-A3B}"
